...
header <- gsub("-mean\\(\\)-X", " x mean", header)
header <- gsub("-mean\\(\\)-Y", " y mean", header)
header <- gsub("-mean\\(\\)-Z", " z mean", header)
header <- gsub("-std\\(\\)-X", " x std deviation", header)
header <- gsub("-std\\(\\)-Y", " y std deviation", header)
header <- gsub("-std\\(\\)-Z", " z std deviation", header)
header <- gsub("-mean\\(\\)", " mean", header)
header <- gsub("-std\\(\\)", " std deviation", header)
header <- gsub("tBodyAcc", "body acc time", header)
header <- gsub("fBodyAcc", "body acc freq", header)
header <- gsub("tGravityAcc", "gravity acc time", header)
header <- gsub("fGravityAcc", "gravity acc freq", header)
header <- gsub("tBodyGyro", "gyroscope time", header)
header <- gsub("fBodyGyro", "gyroscope freq", header)
header <- gsub("Mag", " magnitude", header)
header <- gsub("Jerk", " Jerk", header)
header <- gsub("fBodyBodyAcc", "body acc freq", header)
header <- gsub("fBodyBodyGyro", "gyroscope freq", header)
names(master.set) <- header
